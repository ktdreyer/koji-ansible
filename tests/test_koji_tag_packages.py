...
class TestKojiTagPackages(object):
    ...
    def test_unblock_packages(self):
        ...
        result = koji_tag_packages.remove_package_blocks(
            session, "f34-candidate", check_mode, packages)
        assert result == [
            'unblock pkg ansible',
            'unblock pkg ceph',
            'unblock pkg ceph-ansible',
        ]
        calls = [
            call("f34-candidate", "ansible"),
            call("f34-candidate", "ceph"),
            call("f34-candidate", "ceph-ansible"),
        ]
        session.packageListUnblock.assert_has_calls(calls, any_order=True)

    def test_block_packages(self):
        packages = ["ceph", "ceph-ansible", "ansible"]
        current_packages = [
            {"package_name": "ceph", "blocked": True},
            {"package_name": "ceph-ansible", "blocked": True},
        ]
        ...
        result = koji_tag_packages.ensure_blocked_packages(
            session, "tag", "5", check_mode, packages)
        assert result == ['block pkg ansible']
        session.packageListBlock.assert_called_with("tag", "ansible")
